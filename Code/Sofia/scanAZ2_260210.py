"""
Ver1.1
add hours pick for mutilple run test

Ver 1.0
for the one data point analysis for the day 20260210
"""
...
class slow2FitPWV:
    ...
    def __init__(self):
        ...
        self.hours = [f"{hour:02d}" for hour in range(24)]  # list of hours in the day, used for glob to get the files
    ...
    def getsAzSlowFilenames(self):
        ...
        try:
            ...
            hours = self.hours
            for hour in hours:
                glob_FN = f'{path_save}20260210_{hour}*_scanAz_slow.txt'
                print(f"Looking for files with glob pattern: {glob_FN}")
                for file in glob.glob(glob_FN):
