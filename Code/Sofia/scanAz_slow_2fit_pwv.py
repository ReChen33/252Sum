...
def dat2Am(am_temp_inp = "SPole_annual_50.amc", zen_ang = 45, dat_fn_inp = "test.dat", path = None):
    ...
    time = dat_fn_inp
    if path is not None:
        time = time.split('/')[-1] #except "~/results/20250101_010135_scanAz_slow.dat"
    time = time.split('_')[0]  #example: "20250101T010136.531267_scanAz_slow.dat"
    ...
    if not os.path.exists('time_pwv.csv'):
        with open('time_pwv.csv', 'w') as pwv_file:
            pwv_file.write(f"TIME,PWV TOTAL zenith,PWV TOTAL line-of-sight,zenith angle,{std_com},{nscale_com}\n")
            pwv_file.write(f"{time},{pwv_tot_zen},{pwv_tot_LoS},{zen_ang},{std_value} {std_unit},{nscale_value}\n")
        pwv_file.close()
    else:
...
def slow2Dat(path = None, scanAz_slow_filename = "20250101_010135_scanAz_slow.txt"):
    ...
    num4Time = 0 #clear see the order

    #A for loop to create the dat files based on diff TIME
    #take the values for 10 sec(10 columns) from the Dict

    for i in range(0, len(out_dict), 10):  # process 10 entries at a time
        time, obs_values = list(out_dict.items())[i]  # get the current time and its values
        ...
        dat_filename = f"{time}_scanAz_slow.dat"
        ...
        remove_old_files() #to save space for my laptop

        end = perf_counter()

        #save the time taken to csv file        
        with open('time_take.csv', 'a') as t:
            t.write(f"{time},{end - start:.4f}\n")
        t.close()
...
slow2Dat(path = path_save, scanAz_slow_filename = "20250101_010135_scanAz_slow.txt")
